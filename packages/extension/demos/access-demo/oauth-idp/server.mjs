...
function sendAuthorizeHelpPage(res, title) {
    ...
    res.end(
        `<!doctype html><html><body><h1>${title}</h1>` +
            '<p>Open login via <strong>Cursor MCP OAuth</strong> (&quot;Needs login&quot; on <code>oauth-db2ai-access-demo</code>), not by bookmarking <code>/authorize</code>.</p>' +
            ...
            '<p>Processes: <code>npm run demo:oauth-idp</code>, <code>npm run demo:mcp-oauth:access-demo</code>, access-demo Docker.</p>' +
            '</body></html>'
    );
}
...
function handleAuthorize(req, res, url) {
    ...
    if (responseType !== 'code') {
        if (!responseType && !clientId && !redirectUri && !codeChallenge) {
            sendAuthorizeHelpPage(res, 'access-demo OAuth IDP');
            return;
        }
    ...
    if (!pick) {
        ...
        res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
        res.end(
            `<!doctype html><html><body><h1>access-demo OAuth IDP</h1><p>Login as:</p><ul>${links}</ul></body></html>`
        );
        return;
    }
...
server.listen(PORT, '127.0.0.1', () => {
    console.error(`[access-demo-oauth-idp] http://127.0.0.1:${PORT}`);
    console.error(`[access-demo-oauth-idp] Cursor redirect: ${CURSOR_REDIRECT}`);
    console.error(`[access-demo-oauth-idp] client_id: ${CLIENT_ID}`);
});
