 ...
 *  4. npm audit (workspace root + demos — fails on reported vulnerabilities)
 *  5. generate:all (+ format via demos package.json)
 *  6. build:generated (demos .js for local MCP; not committed)
 *  7. check (format, typecheck, lint)
 *  8. test (language + cli + demos compile check)
 *
...
run('audit (workspace)', 'npm', ['audit']);
run('audit (demos)', 'npm', ['audit', '--prefix', 'packages/extension/demos']);
