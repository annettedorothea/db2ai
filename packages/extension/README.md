...
- Node.js **20+** in demo/tool workspaces (for `@modelcontextprotocol/sdk`, `zod`, `pg` at runtime)
- Docker Desktop for the Pagila demo DB
...
3. In that folder: `npm install` → `npm run db:up` → copy `.env.example` to `.env` → `npm run generate:pagila` (or save `pagila.db2ai` / **Generate tool code**).
4. **File → Open Folder** on the demo workspace.
5. Cursor/VS Code: enable MCP server **`db2ai-pagila`** in `.cursor/mcp.json` (Settings → Tools & MCP).
