...
/** Load `.env` / `.env.local` walking up from each start directory (later files do not override protected keys). */
...
/** Directories to search for `.env` relative to an open document. */
export function workspaceDirsForDocumentUri(documentUri: string): string[] {
    const dirs: string[] = [];
    try {
        if (documentUri.startsWith('file://')) {
            dirs.push(path.dirname(fileURLToPath(documentUri)));
        } else if (path.isAbsolute(documentUri)) {
            dirs.push(path.dirname(documentUri));
        }
    } catch {
        // ignore bad URIs
    }
